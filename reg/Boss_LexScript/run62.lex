...
{@}{dbr}DB_Conn_200_CqBoss
        {|}select PgIdx from Card995PgIdx where id=2{||}[Tag]{-}0
        {|}1
        {|}{\@}{cal}[PgIdx]{||}[PgIdx]{-}PgIdx
        {|}1
{@}{shw}start no: [=]0[=]
{@}{for}[=]0[=]{||}1500000{|}{nfm}9950000000000000{\|}[foridx] 
														 ...
														 {\@}{sql}DB_Conn_200_CqBoss{\|}insert into Card995Info(card,info,intime) values([foridx],'[sys_var_res]',UNIX_TIMESTAMP(NOW())); 
														 																update Card995PgIdx set pgidx=[foridx]+1 where id=2;
                                                        {\|}1
             								 {\@}{shw} 2.save card.[foridx] ok.
             								 {\@}{wtm}100
{@}{cal}[exit]